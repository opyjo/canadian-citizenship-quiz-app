...
export default function PracticeQuizPage() {
      ...
      try {
        ...
        if (category) {
          ...
          if (incorrectData && incorrectData.length > 0) {
            ...
            setError("You don't have any incorrect questions to practice. Take some quizzes first!")
          }
        } else {
  ...
  const finishQuiz = async () => {
    ...
    // Navigate to results
    router.push(
      `/results?answers=${encodeURIComponent(JSON.stringify(selectedAnswers))}&practice=true&practiceType=${practiceType}&category=${encodeURIComponent(categoryName || "")}`,
    )
  }
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex justify-between items-center">
            ...
            <div className="px-3 py-1 bg-red-100 text-red-600 rounded-full text-sm font-medium">
              {practiceType === "category" ? categoryName : "Incorrect Questions"}
            </div>
          </div>
